...
def train(train_loader, model, optimizer):
    ...
    alpha_imerg, alpha_obs = 0.333, 0.667
    ...
    for batch in tqdm(train_loader, leave=False, desc='train'):
        ...
        pred_imerg = pred[imerg_mask]
        gt_imerg = gt[imerg_mask]

        loss_obs = torch.tensor(0.0, device=pred.device)
        loss_imerg = torch.tensor(0.0, device=pred.device)

        if pred_obs.numel() > 0:
            loss_obs = loss_fn(pred_obs, gt_obs)

        if pred_imerg.numel() > 0:
            loss_imerg = loss_fn(pred_imerg, gt_imerg)
...
def main(config_, save_path):
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        if (epoch_val is not None) and (epoch % epoch_val == 0):
            ...
            if val_loss < min_val_obs_loss:
                min_val_obs_loss = val_loss
